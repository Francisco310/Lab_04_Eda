...
class ComparadorPaciente implements Comparator<Paciente> {
    private static final long cambioCategoria = 20;

    public int compare(Paciente p1, Paciente p2) {

        long prioridad1 = p1.getCategoria() - (p1.getTiempoEspera() / cambioCategoria);
        long prioridad2 = p2.getCategoria() - (p2.getTiempoEspera() / cambioCategoria);

        prioridad1 = Math.max(1, prioridad1);
        prioridad2 = Math.max(1, prioridad2);


        if ((p1.getCategoria() == 4 && p2.getCategoria() == 5) && prioridad1 == prioridad2) {
            return -1;
        }
        if ((p1.getCategoria() == 5 && p2.getCategoria() == 4) && prioridad1 == prioridad2) {
            return 1;
        }

        if (prioridad1 != prioridad2) {
            return Long.compare(prioridad1, prioridad2);
        }
...
class Simulacion{ // Clase que simula una jornada completa de 24 horas
    ...
    public void simular(int pacientesPorDia) {
        ...
        while (minActual < 1440 || indicePaciente < pacientes.size()) {
            ...
            // Incrementa tiempo de espera para todos los pacientes en la cola
            for (Paciente paciente : hospital.getcolaAtencion()) {
                paciente.incrementarEspera();
            }

            // Reconstruye la cola para que se actualice el orden con la nueva prioridad efectiva
            PriorityQueue<Paciente> nuevaCola = new PriorityQueue<>(new ComparadorPaciente());
            nuevaCola.addAll(hospital.getcolaAtencion());
            hospital.getcolaAtencion().clear();
            hospital.getcolaAtencion().addAll(nuevaCola);
